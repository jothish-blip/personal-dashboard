import { sendToServiceWorker } from "@/notifications/engine/useNotificationSystem";
import { NexModule } from "@/notifications/types/types";
...
export const handlePlannerEvent = (addNote: any, event: any, type: "1h" | "10m" | "now") => {
  ...
  routeNotification(addNote, "planner", title, body, "/Planner", priority);
};
...
export const handleWorkspaceAction = (addNote: any, type: 'save' | 'deepWork' | 'nudge') => {
  if (type === 'nudge' && canNotify("ws_nudge", 24 * 60 * 60 * 1000)) {
    routeNotification(addNote, "mini", "Review Your Ideas 💡", "You have pending notes in your workspace. Organize your thoughts.", "/Workspace", "low");
  }

  if (type === 'save' && canNotify("ws_autosave", 30 * 60 * 1000)) {
    routeNotification(addNote, "mini", "Workspace Synced 💾", "Your thoughts and documents are secured.", "/Workspace", "low");
  }

  if (type === 'deepWork' && canNotify("ws_deepwork", 45 * 60 * 1000)) {
    routeNotification(addNote, "focus", "Deep Work Pattern 🧠", "Heavy productivity detected in the workspace.", "/Workspace", "medium");
  }
};
