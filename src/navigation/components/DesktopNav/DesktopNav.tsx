...
export default function DesktopNav(props: DesktopNavProps) {
  ...
  // 🔥 Bulletproof fallback: If parent explicitly passes `null`, this prevents the crash
  ...
  return (
    <>
      ...
      <div className={`hidden md:flex h-[64px] items-center px-6 max-w-[1500px] mx-auto w-full relative z-50 transition-colors duration-300 ${
        isDarkMode ? "bg-[#050505]/90 border-b border-gray-800" : "bg-white/90 border-b border-gray-100"
      }`}>
        ...
        <div className={`mx-5 w-px h-6 transition-colors ${isDarkMode ? "bg-gray-800" : "bg-gray-200"}`} />

        {/* NAV (DRAG & DROP) */}
        <div className="flex gap-2 relative">
          {navItems.map((item, index) => {
            const Icon = item.icon;
            // 🔥 Use safePaths to prevent crashes
              ...
              >
                <button
                  ...
                  className={`relative flex items-center justify-center gap-2 px-3.5 py-2 rounded-xl text-xs font-semibold tracking-wide transition-all duration-300 ease-out hover:-translate-y-[1px] ${
                    isActive
                      ...
                      : isDarkMode ? "text-gray-400 hover:bg-[#1a1a1a] hover:text-white" : "text-gray-600 hover:bg-gray-50 hover:text-black"
                  }`}
                >
                ...
                {isCompact && (
                  <div className={`absolute top-full left-1/2 -translate-x-1/2 mt-2 px-2.5 py-1 rounded-md text-[10px] font-bold opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none shadow-md z-50 ${
                    isDarkMode ? "bg-[#1a1a1a] text-gray-300 border border-gray-800" : "bg-gray-900 text-white"
                  }`}>
                    {item.label}
                  </div>
                )}
        ...
        <div className="flex items-center gap-4">
            ...
            <div className="relative flex items-center">
              <button
                ...
                className={`relative p-2 transition-all duration-300 rounded-full hover:-translate-y-[1px] ${
                  isDarkMode ? "text-orange-400 bg-orange-950/20 hover:bg-orange-950/40 border border-orange-500/20" : "text-orange-500 bg-orange-50 hover:bg-orange-100 border border-orange-200"
                }`}
              >
            ...
            {isProfileOpen && (
              <>
                ...
                <div className={`absolute right-0 mt-3 w-56 border rounded-xl shadow-2xl py-2 z-50 animate-in fade-in zoom-in-95 duration-200 ${
                  isDarkMode ? "bg-[#111111] border-gray-800 shadow-black/50" : "bg-white border-gray-100 shadow-gray-200/50"
                }`}>
                  <div className={`px-4 py-3 border-b mb-1 ${isDarkMode ? "border-gray-800" : "border-gray-50"}`}>
                    <p className={`text-sm font-bold truncate ${isDarkMode ? "text-white" : "text-gray-900"}`}>
                      {userProfile?.full_name || "User"}
                    </p>
                    <p className={`text-[10px] truncate mt-0.5 uppercase tracking-widest font-bold ${isDarkMode ? "text-gray-500" : "text-gray-400"}`}>
                      {userProfile?.email || ""}
                    </p>
                  </div>
          ...
          <button 
            ...
            className={`p-2 ml-2 rounded-xl transition-all duration-300 hover:-translate-y-[1px] border ${
              isDarkMode 
                ? "bg-[#111111] border-gray-800 text-gray-500 hover:text-white hover:border-gray-600" 
