...
function useDiaryInsights(allEntries: Record<string, DiaryEntry>, currentEntry: DiaryEntry, consistency: number, actualToday: string, allDates: string[]) {
  ...
  return useMemo(() => {
    ...
    if (last7Entries.filter(e => (e.tags || []).includes('Distracted')).length >= 3) patterns.push("Distraction pattern detected (3+ times this week)");
    ...
    else if ((currentEntry.tags || []).includes('Distracted') && consistency < 40) aiInsight = "Distraction pattern active. Environment reset required.";
...
export function useDiarySystem() {
  ...
  const pushNotification = useCallback((msgKey: string, title: string, desc: string, type: NotificationType, url: string) => {
