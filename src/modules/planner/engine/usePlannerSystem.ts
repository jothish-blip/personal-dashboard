...
export function usePlannerSystem() {
          ...
          if (event.status === "pending" && event.date <= todayStr) {
            ...
            if (eventDateTime < now) {
              ...
              safeNotify(`missed_notified_${event.id}`, () => {
                addNotification('planner', 'Missed Event ⚠️', `You missed: ${event.title}`, 'high', '/Planner');
              });
  ...
  const handleSave = () => {
    ...
    if (formData.id) {
      ...
      addNotification('planner', 'Event Updated', `Modified: ${formData.title}`, 'low', '/Planner');
    } else {
      ...
      addNotification('planner', 'Event Scheduled', `${formData.title} added at ${formData.time}`, 'low', '/Planner');
    }
    ...
    setEvents(prev => prev.map(e => {
      if (e.id === id) {
        ...
        if (nextStatus === "completed") {
          addNotification('planner', 'Event Completed ✅', `${e.title} marked as completed`, 'low', '/Planner');
        }
        return updatedEvent;
      }
      return e;
  ...
  const deleteWithUndo = (id: string) => {
    ...
    addNotification('planner', 'Event Removed', `${target?.title} deleted from schedule`, 'medium', '/Planner');
  };
  ...
  const rescheduleTask = (id: string) => {
    setEvents(prev => {
      ...
      if (slot) {
        ...
        addNotification('planner', 'Event Rescheduled 🔄', `${updatedEvents[taskIndex].title} moved to ${slot.time}`, 'medium', '/Planner');
      }
