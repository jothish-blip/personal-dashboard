...
interface GridProps {
  ...
  actualToday: string;
  todayRef: React.RefObject<HTMLTableCellElement | null>; // <-- Just add " | null" here
...
export default function Grid({
  ...
  // Manage selected week and swipe states locally to prevent parent re-renders
  ...
  const handleTouchStart = (e: React.TouchEvent) => {
    setTouchEndX(null); setIsSwiping(false);
  ...
  const handleTouchEnd = () => {
    ...
    if (distance > 50 && selectedWeek < weeksInMonth.length - 1) setSelectedWeek(selectedWeek + 1);
    if (distance < -50 && selectedWeek > 0) setSelectedWeek(selectedWeek - 1);
    setTouchStartX(null); setTouchStartY(null); setTouchEndX(null); setIsSwiping(false);
  };
                            ...
                            return (
                              <div key={day} className={`relative flex flex-col items-center justify-center gap-1.5 py-2 rounded-xl flex-shrink-0 touch-manipulation ${heatmapBg} ${isToday ? "bg-orange-100 border border-orange-300" : "border border-transparent"}`} title={tooltipMsg}>
                                ...
                                <input
                                  ...
                                  checked={isDone}
                                  onChange={() => { if (!isDisabled) handleToggleSafe(task, dateStr); }} 
                                  className={`w-6 h-6 rounded-md z-10 ${heatmapAccent} ${glowClass} ${isDisabled ? "opacity-30 grayscale cursor-not-allowed" : "transition-transform cursor-pointer"}`}
                                />
                              </div>
                            );
                          })}
